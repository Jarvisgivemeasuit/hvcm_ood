...
def get_args_parser():
    parser = argparse.ArgumentParser('HVCM', add_help=False)
...
def train(args):
    ...
    dataset = Imagenet('train', args.data_path, args.num_labels, transform)
    ...
    teacher = utils.MultiCropWrapper(
        teacher,
        utils.DINOHead(embed_dim, args.out_dim, args.use_bn_in_head, 
                 num_kernel=args.num_kernel),
    )
    ...
    # --- Init HVCM loss ---
    center_loss = HVCMLoss(args.local_crops_number + 2, 
                                    num_classes=args.num_labels,
                                    feat_dim=[args.num_kernel, args.out_dim // args.num_kernel]).cuda()
    # ----------------------
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, center_loss, data_loader,
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            centerloss = center_loss(gmm_weights, student_output, labels, args)
...
class HVCMLoss(nn.Module):
    ...
    def forward(self, gmm_weights, x, labels, args):
        """
        Args:
        ...
        for i in range(2):
            ...
            '''loss for center'''
            kl_x = F.kl_div(F.log_softmax(x[i],dim=-1), F.softmax(center.clone().detach(), dim=-1), reduction='none').sum(-1) * args.alpha
            kl_ce = F.kl_div(F.log_softmax(center, dim=-1), F.softmax(x[i].clone().detach(),dim=-1), reduction='none').sum(-1) * args.beta
            ...
            for cls in labels.unique():
                self.gmm_weights[cls] = F.softmax(gmm_weights[i][labels==cls], dim=-1).detach().mean(0) * args.gamma + \
                                        self.gmm_weights[cls] * (1 - args.gamma)
        return total_loss / 2


if __name__ == '__main__':
    parser = argparse.ArgumentParser('HVCM', parents=[get_args_parser()])
